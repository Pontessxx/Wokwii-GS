...
#define DHTPIN 2 	//porta do dht22
#define DHTTYPE DHT22	//tipo do dht
DHT dht(DHTPIN, DHTTYPE);


#include <LiquidCrystal_I2C.h> //lcd biblioteca
LiquidCrystal_I2C lcd(0x27, 16, 2);  // Endereço I2C do display, 16 colunas e 2 linhas

int D4 = 2;
int D32 = 32; // iniciando os leds do "semaforo" - VERDE
int D33 = 33; // iniciando os leds do "semaforo" - AMARELO
int D25 = 25; // iniciando os leds do "semaforo" - VERMELHO
...
{   
    ...
    if (temperatura <=25 || temperatura >=38){
      digitalWrite(D32, LOW);
      digitalWrite(D33, LOW);
      digitalWrite(D25, HIGH);
    }else if ((temperatura >=35 && temperatura <=38)|| (temperatura >=25 && temperatura <=30)){
      digitalWrite(D32, LOW);
